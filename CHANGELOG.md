...
- **460 tests** (up from 426), thirty-four of them new — one case per literal, so a batch that loses one still passes on the others.
- **Guard proved discriminant**: restoring the real pin reddens exactly four cases, each naming its own file and line.
