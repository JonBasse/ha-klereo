...
### Verified
- **545 tests** (up from 477): twenty-two new for the button, twenty-eight for the energy sensors, eighteen for the auto-off timer.
...
- 🔴 **The auto-off tests are discriminant, and the two that carry the ticket are negative controls.** Five mutants were run against them: defaulting an absent `offDelay` to `0` in the model reddens **three** cases; dropping the presence gate so every output gets a timer reddens **one**; sending the raw float straight to the wire reddens **one**; skipping `_async_confirm_command` so a queued command reads as a success reddens **two**, including the `status 13` rejection; and lowering the floor to `0` — the unmeasured value — reddens **two**, the README agreement among them.
