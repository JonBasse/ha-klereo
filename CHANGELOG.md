...
- **The `number` water setpoint and the `climate` thermostat are gated on DIFFERENT things, on purpose** ([#172](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/172)). No behaviour changed, and none should: the ticket opened describing the two as "disagreeing about output 4", and reading the two write paths showed they never speak about the same object. `number` writes `SetParam.php` with `poolID`, `paramID` and `newValue` — **no output index anywhere on that path** — so it is gated on `is_setpoint_offered` (access, `HeaterMode`). `climate` exists only where output 4 is reported, because its modes write `SetOut.php` with `outIdx: 4`. A setpoint is a parameter of the **pool**; a heat mode is a property of an **output**.
  ...
  - ⚠️ **One question stays open and does not block any of this**: whether a box with a `HeaterMode` but no output 4 actually *acts* on a `ConsigneEau` write. The parameter is real there — the bench reports `EauMin: 0` / `EauMax: 40` — but nobody has written to it in that configuration. Measuring it would change this paragraph, not the gates: whichever way it lands, removing the entity would be wrong and inventing a thermostat would be wrong.
