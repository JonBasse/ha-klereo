...
- **A speed control for variable-speed ("analogue") Filtration pumps** — a new `number.filtration_speed` entity on output 1, bounded `0..PumpMaxSpeed`, always writing `newMode=Manual`.
  - **Source: the upstream Jeedom plugin.** Klereo's `GetPoolDetails` carries a top-level `PumpMaxSpeed` field — undocumented by Klereo itself (`docs/klereo-api.md`'s field lists are elided, réserve 1) but read by [`MrWaloo/jeedom-klereo`](https://github.com/MrWaloo/jeedom-klereo)'s `klereo.class.php`: `PumpMaxSpeed > 1` marks an "Analogic pump" (l.632, 912, 1429), and upstream exposes a slider bounded `0..PumpMaxSpeed` whose value is sent verbatim as `newState` under `newMode=Manual` (l.1493-1495, 1511-1513). This entity ports that mechanism — a per-installation range read from the wire, not a fixed menu.
    - An earlier draft of this feature hardcoded three named speeds (`Off`/`Speed 1`/`Speed 2`/`Speed 3`) after [kouakattak/Klereo_HA](https://github.com/kouakattak/Klereo_HA), an independent community integration whose `select.py` happens to send `newState` 1/2/3 for a fixed three-speed pump. That shape is superseded by the upstream mechanism above, which generalises to any pump's own maximum rather than assuming three steps for everyone.
  - ✅ **`PumpMaxSpeed` is confirmed on a real installation** — a reporter's Home Assistant diagnostics export carries `"PumpMaxSpeed": 3` on an analogue Filtration pump, output 1. Until then it was read on the upstream plugin's testimony alone, the same evidentiary status `SetParam.php` and `SetAutoOff.php` already carry in this codebase (shipped since #128 / #162 on that basis). The entity is only ever created when a payload actually carries the field AND reports it above 1 (`number._extract_numbers`) — never invented, the same rule `KlereoAutoOffNumber` already follows for `offDelay`. An installation with a fixed-speed (or no) Filtration pump gets no entity here; `switch.KlereoSwitch`'s plain ON/OFF already covers it, unchanged.
  - **Kept as a separate entity from the existing Output Mode select on purpose.** `select.KlereoOutputModeSelect` already owns Manual / Time Slots / Timer / Regulation / … for every output including this one; folding speed into the same control would give two entities the power to disagree about the output's mode.
  - **One narrowly-scoped consistency fix rides along**, needed so switching this output's mode away from and back to Manual does not silently mislead the new entity: `select.KlereoOutputModeSelect._state_for_mode` no longer binarizes output 1's preserved state to plain ON/OFF when returning to Manual — it preserves whatever speed step was last read. Scoped to output 1 alone; every other output keeps the original ON/OFF-only preservation.
  - 🔴 **`switch.KlereoSwitch` deliberately does NOT follow the speed under Manual.** An earlier draft made it read any non-zero Manual status as "on"; reverted on request, so the switch stays exactly what it was before this feature existed — plain `status == OUT_STATE_ON` (1) — and the speed entity (above) is where a speed step actually shows. Only Regulation is special-cased, see below.
  - If you own a variable-speed Filtration pump, please confirm on the tracking issue that a given speed step on the slider does what its number suggests on your hardware — that is the one part of this feature nobody here has independently measured yet.

### Fixed

- ✅ **The Filtration switch now reads *on* while the box is regulating — confirmed on a live installation, mode switched from Manual to Régulé in the running app and the switch followed immediately on refresh.** A reporter's first Home Assistant diagnostics export surfaced the underlying fact: on an analogue pump (`PumpMaxSpeed: 3`), Regulation (`mode: 3`) reported `status` AND `realStatus` both pinned to **2** at that moment — not the relay's real state — while the reporter's own Klereo app showed Filtration running. `switch.KlereoSwitch` reading `status` literally under Regulation therefore showed **off** on a pump that was demonstrably on.
  - **Fixed for the Filtration output specifically**: Regulation now reads as always on, the same simplification the Heating switch already documents for "not stopped" — an approximation of the MODE, not a decoded relay state. Scoped to Regulation on the Filtration output alone; every other non-Manual mode on this output, and every other output under Regulation, keep the original `status == 1` reading.

- ✅ **The Filtration speed now reports a value under Regulation too — `realStatus`, not `status`.** FOUR further diagnostics exports from the same reporter, all still under Regulation, refuted the first capture's apparent pattern: `status` alone does not track the live speed. The clearest shows `status: 1` next to `realStatus: 2`, `realStatus` agreeing with `ExtraParams.PmpRunningSpeed` and with an externally measured jump in the pump's power draw (~80 W → ~360 W) that the reporter read off their own energy meter — `status` did not move with either. `models.KlereoOutput.real_status` carries the full citation.
  - ⚠️ **One of the five captures briefly looked like a contradiction, and it wasn't one.** Right after the reporter lowered a regulation setpoint, `realStatus` had already dropped while `ExtraParams.PmpRunningSpeed`/`PmpWatts`/`PmpRPM` were still at the OLD, higher reading — `realStatus` is the box's current TARGET and updates promptly, `Pmp*` is the physically measured reality and LAGS during a transition. A further export taken a couple of minutes later showed both had converged. Documented on `const.PUMP_TELEMETRY_TYPES` so the next reporter does not re-discover this as a bug.
  - `number.filtration_speed` now reads `real_status` under Regulation (previously: nothing, `native_value: None`, because the first capture alone looked like `status` was pinned to a constant "auto" flag — that reading is now known to be a coincidence of one sample, not a rule). Manual is unaffected and still reads `status`.
  - `real_status` is parsed onto `KlereoOutput` for exactly this reason — the same `off_delay` exception (#162): a FEATURE reads it, so it is no longer left as noise for the raw diagnostics export alone. Defaults to `None`, never `0`, so an installation that sends no `realStatus` reports nothing rather than a false "stopped".
  - ⚠️ Still scoped to Regulation on the Filtration output alone, and still resting on ONE installation's diagnostics — five captures now, all self-consistent once the transition lag above is accounted for. `switch.KlereoSwitch` is unchanged by this entry — it already read Regulation as always-on regardless of either field.

### Added

- **Filtration pump telemetry sensors**, read from `ExtraParams.Pmp*` — `Filtration Pump Power`, `Running Speed`, `Speed Setpoint`, `RPM`, `Flow`, `Status`, `Error Code`, `Timeout`. Requested alongside the `realStatus` fix above, from the same reporter's diagnostics exports. Plain `sensor` entities in the main entity list, same as everything else this integration exposes; each has its own icon rather than the generic default.
  - 🔴 **UNDOCUMENTED everywhere this project otherwise looks**: absent from `docs/klereo-api.md` and read by NOTHING in the upstream Jeedom plugin, which names no `Pmp*` key at all. Sourced entirely from one reporter's own exports, `const.PUMP_TELEMETRY_TYPES` carries the full citation.
  - Reach `sensor` through the existing curated `PARAM_NAMES` gate (`_extract_sensors`) — the same mechanism every other `params`/`ExtraParams` sensor already uses, so no new extraction code was needed. Created only for the keys a given payload actually carries; an installation without this telemetry gets none of them, same as every other gate in this file.
  - ⚠️ **A first draft grouped all eight under Home Assistant's `diagnostic` entity category, reverted on request.** This project has never used that category anywhere else, and Power/Running Speed in particular are numbers a reporter is likely to want on the main dashboard rather than behind a collapsed section.
  - ✅ **`PmpSpeed` and `PmpRunningSpeed` carry a `%` unit — confirmed by the reporter directly**, not inferred from the captured 0-100 range the way an earlier draft of this entry put it. `PmpFlow` and `PmpRPM` stay unitless: the reporter did not confirm either, so this project does not extend the same confidence to them on the strength of a plausible-looking name or range alone.
  - ✅ **`PmpTimeout` (the one field that changed mid-investigation) is resolved as a self-clearing transient, not an open fault.** It went `0` → `1` across a regulation setpoint change and stayed `1` through the speed transition (previous entry), then returned to `0` on its own a few minutes later — confirmed in Home Assistant's own history. The reporter found no trace of it anywhere in the Klereo app, no alert and no indicator. What specifically triggers it remains unknown, but "does it stay stuck" is answered: no.
  - ⚠️ `PmpStatus` is exposed on the reporter's own "presumably on/off" reading — a guess they offered themselves, not something confirmed elsewhere. Kept as a raw number rather than turned into a `binary_sensor` on the strength of "presumably".
...
- **`realStatus` is the physical state of a relay and `status` is the commanded one — measured, written down, and `switch`/`select` still read `status`** ([#141](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/141)). No behaviour changes: this ships the answer to a question that had been open since 2026-08-28, and the reasons the answer does not license a swap.
  ...
  - 🔴 **The run times are a second witness, not a restatement of the first.** Where a `params` counter names the same equipment, it and `outs[].totalTime` carry the same value: `PHMinus_TotalTime` equals `outs[2].totalTime` exactly (`127218`, a stopped output whose counter cannot drift) and `Filtration_TotalTime` matches `outs[1].totalTime` to 351 (a running one, which does). `Chauff_TotalTime` is `0`, like both divergent outputs.
    - ⚠️ **That correspondence is measured on three equipments and is NOT a general identity — the fourth contradicts it.** `params` carries only four `*_TotalTime` keys; output 15 counts `7680`, no `params` key equals `7680`, and `ElectroChlore_TotalTime` — the only plausible partner — reads `0`. Writing it as an identity would repeat, one paragraph away, the mistake this same entry corrects about the key count: taking a reading for a shape. The narrower measured claim is enough for the argument, since both divergent outputs are heat-typed (`type: 8`) and `Chauff_TotalTime` is `0` like them. **Where the two fields disagree, `realStatus` agrees with the counter and `status` does not** — and that argument stands on its own, without interpreting the `Aq*` fields no source names.
...
- 🔴 **A confirmed command now reaches every entity of that output, not only the one you touched** ([#174](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/174)). Starting the heat pump from the thermostat or from **Heating Mode** left the **Heating** switch reading `off` for up to ten minutes, on a pump that had physically restarted and that Klereo's own client already showed as heating. Measured by [@nopbop](https://github.com/JonBasse/ha-klereo/issues/55) on 2026-09-08.
  - **The cause is local to this integration, and that took a measurement to establish.** `select` and `switch` each wrote their OWN state optimistically before sending, so whichever entity you actuated moved at once and its siblings waited for the next poll — five minutes by default, ten on the box's own refresh period. The competing reading was that Klereo answers with a partially fresh payload, `mode` updated and `status` not, which would have made this a fact about the box that no amount of local optimism should hide. It is refuted: in the diagnostics export of 2026-09-08, `mode` and `status` are two fields of **one** `outs[]` element carrying **one** `updateTime` (`684` on output 4), so they cannot have different freshness.
  - **One source of optimism, in the coordinator.** A command names an *output*, not the entity that sent it, and the coordinator is the only place that sees `switch`, `select` and `climate` at once. Writing it three times is what [#166](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/166) already paid for with the KlereoTherm state table, which is now shared rather than copied.
  - 🔴 **It is written into the typed model and is overwritten by the next payload, never sticky.** `_async_update_data` rebuilds the models from scratch on every poll, so a payload that *contradicts* the optimism wins. That is the constraint the ticket refused to build without: an optimism outliving a contradicting refresh would mask a genuine cloud-side lag rather than remove one, and the export that refuted that reading covers exactly one installation. A test turns red on stickiness, and two more turn red if the optimism spills onto a neighbouring output or another pool.
  - 🔴 **`AUTO` is never written into `status`.** Only Manual states a relay; every other mode sends `AUTO`, which means "the box decides" and says nothing about the relay. The same export shows output 1 in Regulation reporting `status: 1` — a running relay under automatic control — so echoing the commanded `AUTO` would have flipped a running switch to `off` and invented the very lie this fix removes.
  - **Only a confirmed command is shown.** Unconfirmed is not a verdict ([#140](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/140)); painting one across three entities would restore the silence [#95](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/95) exists to remove, where a rejection reads exactly like a success.
...
- **`docs/klereo-api.md` now describes `outs[]`, which the official source elides entirely** ([#141](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/141)). The full key table, the `realStatus` / `status` verdict with the payload that carries it, and the upstream reading.
  ...
  - ✅ **The unit of `outs[].totalTime` is narrowed, not closed.** The analogy with `params.<Equipment>_TotalTime` is no longer an analogy where the two meet — three equipments out of four carry the same value on both sides — so the remaining inference is at the other end of that equality: seconds comes from upstream dividing by 3600 for hours, corroborated by `Filtration_TodayTime: 30511` being 8 h 28 as seconds and an impossible 21 days as minutes.
...
- **An auto-off timer for every output** ([#162](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/162)). `number.<pool>_<output>_auto_off_timer` exposes Klereo's *Temps minuterie* — how long an output runs before switching itself off — in **minutes**, adjustable from 1 to 600. It is the last capability the upstream Jeedom plugin had and this integration did not, found by the three-source crossing of [#147](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/147).
  - 🔴 **`SetAutoOff.php` was attested by upstream code and never by a reply from `connect.klereo.fr`** — this ticket was blocked on that for five days, and on nothing else. It was settled on 2026-09-08 by a two-stage probe designed to change nothing: a call naming **no output** answered `{"status":"error","detail":"Mauvais délais"}` — **a route that does not exist does not *validate* an `offDelay`, it 404s** — and an idempotent rewrite of `240 → 240` returned `cmdID 4399790`, then `status: 9, detail: "Ok"`. The relevé is in `docs/klereo-api.md` § *Poser un délai d'extinction automatique*.
  - ✅ **The account that measured it is at `access: 10`**, so this endpoint asks for **no professional access** — which was the real question to settle before building. An entity that answered `status 13` for most users would not have been worth having.
  - 🔴 **An output whose payload carries no `offDelay` gets NO entity** — not one pinned at `0`, not a fallback. The five outputs of one installation all carry the field, and nothing says that is universal; `plans` already is not ([#128](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/128), [#135](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/135), [#138](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/138)).
  - **The write is confirmed, never assumed.** `SetAutoOff` only *queues* and returns a `cmdID`; the verdict comes from `CommandStatus` through `_async_confirm_command`, like every other write since [#95](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/95). An HTTP 200 says nothing — `status 13` reads exactly like success on the wire.
  - ⚠️ **`KlereoOutput` grew a field, and that is not a reversal of [#145](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/145).** #145 refused to widen the model *to make payload fields visible* — the raw diagnostics export already does that, and a field nothing reads is noise. This one is read by a feature, and the coordinator hands the platforms typed models rather than raw dicts, so it has to be carried there. The guard test that held #145's line was **re-aimed rather than deleted**: it now enumerates the allowed fields and still asserts that `realStatus` — read by the export and by nothing else — stays out.
  - 🔴 **Two things about this timer stay UNMEASURED, and the integration asserts neither.** What `offDelay: 0` does — disable the timer, or be refused? The floor offered is `1`, which is upstream's own declared floor, so only values known to be accepted are offered and nothing is claimed about the one that is not. And what the timer *means* on an output that is not in Manual mode: one installation's output 1 runs under regulation and still reports `offDelay: 5`. The entity is deliberately **not** hidden, disabled or annotated there — all three would answer a question nobody has asked the box. Both are written in `README.md`, because silence would itself be the claim.

### Documented
- **The `number` water setpoint and the `climate` thermostat are gated on DIFFERENT things, on purpose** ([#172](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/172)). No behaviour changed, and none should: the ticket opened describing the two as "disagreeing about output 4", and reading the two write paths showed they never speak about the same object. `number` writes `SetParam.php` with `poolID`, `paramID` and `newValue` — **no output index anywhere on that path** — so it is gated on `is_setpoint_offered` (access, `HeaterMode`). `climate` exists only where output 4 is reported, because its modes write `SetOut.php` with `outIdx: 4`. A setpoint is a parameter of the **pool**; a heat mode is a property of an **output**.
  - **The internal proof is in `climate.py` itself**, which already calls both gates separately: `async_set_hvac_mode` goes through output 4, `async_set_temperature` through `is_setpoint_offered` — the same gate as `number`. A thermostat is a **composite** object, and aligning the gates would break that composition.
  - 🔴 **Both "corrections" the original ticket weighed destroy something.** Barring `number` on output 4 **deletes** a writable setpoint from an installation that has a real one — the shape of [#128](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/128) / [#135](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/135). Creating `climate` without output 4 **invents** a thermostat whose mode control writes to an output the box does not report. Doing nothing is not the soft option here; it is the only one that destroys nothing.
  - **Nobody loses a capability.** The measured bench carries `outs` = 0, 1, 2, 3, 9 with `HeaterMode: 1` and `access: 10`: a writable water setpoint and no thermostat. Every output it does have already carries a `switch` and a `select`; the thermostat is only a comfortable wrapper around output 4, and where there is no output 4 there is nothing to wrap.
  - ⚠️ **One question stays open, and it now has a named mechanism rather than a blank** (updated 2026-09-08): whether a box with a `HeaterMode` but no output 4 actually *acts* on a `ConsigneEau` write. The parameter is real there — the bench reports `EauMin: 0` / `EauMax: 40` — but nobody has written to it in that configuration. **What changed is why the configuration exists at all.** It read as a leftover; it is not one. The `plan64` survey of 2026-09-08 (`docs/klereo-api.md` § *La programmation horaire*) found the bench's output 9 — *Aux 4* — programmed 06h15→21h00, and its owner names that output as **his heat pump**. A third-party heat pump on an auxiliary relay is all-or-nothing and lives on whatever output it is wired to; output 4 is the **KlereoTherm**, Klereo's own product with its heat modes. The two coexist, and `OUT_IDX_HEATING = 4` is not weakened by either. So a box regulating water temperature through a pump on Aux 4 explains `HeaterMode: 1` and a bounded, writable `ConsigneEau` without any output 4 — these values have real machinery behind them. 🔴 **This does not close the question**: a plausible, named mechanism is not a measurement. Nobody has yet written a setpoint on that installation and observed an effect. Measuring it would still change this paragraph and not the gates — whichever way it lands, removing the entity would be wrong and inventing a thermostat would be wrong.
...
- **The `plan64` schedule decoding is settled, and the previous survey's two negative claims were WRONG.** `docs/klereo-api.md` § *La programmation horaire* said "the three `plan64` present decode to all zeros" and "on the bench, outputs 2 and 9 have no entry". A second probe on the same installation, 2026-09-08, read `plans` in full: it carries **ten** entries, `plans[0..9]`, and `plans[9]` — output 9, *Aux 4* — is **programmed**. The 2026-09-03 survey never enumerated the container; its negative conclusion rested entirely on that partial read, which is why it is corrected in place rather than quietly replaced.
  - ✅ **The bit order is decided by a STRUCTURAL property, not by which decoding looked nicer.** `plans[9]` is `AAAA/v///////w8A` — twelve bytes, **59 bits set**. Under "low nibble first, LSB→MSB" those 59 bits form **one contiguous interval**, 06h15 → 21h00: span 59 equals the bit count, zero holes, one block. The three other candidate orders scatter the same 59 bits into two or three blocks with 1 or 5 holes. A wrong transcription has no reason to produce a contiguous schedule.
  - ✅ **The owner confirms the schedule**, asked before being shown the result: *Aux 4* is his **heat pump**, which "follows the automatic programming", and "yes, normally" from 06h15 to 21h00.
  - ⚠️ **The strength of that testimony is recorded because it is not total** — "à priori", "normally". It is a **confident but not certain** confirmation. The hard half is the structural property; the human half is the clock. **Together they are the measurement — neither alone is.** The testimony alone would not separate 06h00 from 06h15; the structure alone would not say these bits are a real schedule rather than a lucky pattern.
  - ⚠️ **He switched that pump off by hand last week pending winterisation, and this contradicts nothing.** `plan64` is the **stored** programme, not the output's live state — a schedule survives a manual shutdown, which is exactly why it is still readable while the equipment is idle. Reading "he turned it off" as an objection confuses the timer with the relay.
  - 🔴 **Still read-only, and this changes nothing about that.** No endpoint in any of the three sources writes a schedule. The decoding makes reading possible; it does not make writing possible.

### Verified
- **552 tests** (up from 477): twenty-two new for the button, twenty-eight for the energy sensors, six for the disjoint gates, nineteen for the auto-off timer.
...
- 🔴 **Seven mutants were run against the auto-off tests, and every one of them reddened**: defaulting an absent `offDelay` to `0` in the model reddens **two**; dropping the presence gate so every output gets a timer reddens **one**; sending the raw float straight to the wire instead of an integer reddens **one**; skipping `_async_confirm_command`, so a merely *queued* command reads as a success, reddens **two** — the `status 13` rejection among them; lowering the floor to `0`, the unmeasured value, reddens **two**, including the README agreement; making the entity ignore the refresh reddens **one**; and un-narrowing `available`, so a vanished output stays pinned to its last reading, reddens **one**.
- **The `0`-default mutant is why the presence gate is controlled TWICE.** The first two controls build a `KlereoOutput` by hand, so neither of them can see the *parser* default an absent `offDelay` to `0` — which is exactly how that gate stops working while still looking guarded. The third goes through `KlereoPoolDetails.from_dict`, the call the coordinator actually makes, and it is the one the mutant catches.
- Of the twenty-eight energy tests, **six are negative controls** — the ones that carry the ticket.
...
- 🔴 **The heat pump's `newState` is now the one the official client sends, and it is decided by the mode you are switching TO** ([#166](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/166)). Setting output 4 to **Heating** sent `newState = 2` (Auto); the Klereo web client sends `1` (On). Switching to Auto and stopping the pump were already right. Measured on the wire — three captures by [@nopbop](https://github.com/JonBasse/ha-klereo/issues/55) and two by [@StephanH27](https://github.com/JonBasse/ha-klereo/issues/55), on two installations, 2026-09-06.
  ...
  - **The other half of that capture is now settled too**, by a further measurement on 2026-09-07 — see the water-setpoint entry below ([#170](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/170)). The question was whether the box accepts a setpoint write while the value reads the `-2000` sentinel. It does.

- 🔴 **A disabled water setpoint no longer makes itself permanently undisableable** ([#170](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/170)). Where `ConsigneEau` read Klereo's `-2000` "setpoint disabled" sentinel, the integration created no `number` entity, withheld the thermostat's target-temperature dial and refused the write outright. Since nothing could write the value, the value stayed `-2000` — **the guard produced the very condition it claimed to respect**, and that loop is why [#55](https://github.com/JonBasse/ha-klereo/issues/55) stayed stuck for a month.
  - **Measured on 2026-09-07** by [@StephanH27](https://github.com/JonBasse/ha-klereo/issues/55) on an installation carrying the sentinel: starting the pump from the official v1 client queues two commands, and **both** `WaitCommand` responses answer `status: 9, detail: "Ok"` — so whichever of the two carried the `SetParam`, the box accepted it. The box's own front panel corroborates it, going from `Arrêté` to **`25.0 °C`** — exactly the `newValue` of the captured `SetParam` — and staying there after the pump was stopped again.
  - **What was refuted is a written claim, not an oversight.** The refusal's stated reason was that "the write would be discarded by the box". That reasoning was correct for [#124](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/124) — an on/off heater accepts *Cooling*, answers status 9 and does nothing — and it was carried here **by analogy rather than by measurement**. The analogy does not hold.
  - **The sentinel is a VALUE, not a permission**, and the fix separates the two questions everywhere they were conflated. The entity now exists and reads `unknown`; the dial is offered; the write goes through. Reading is unchanged — `-2000` is still never rendered as a temperature.
  - ⚠️ **Deliberately narrow: the three other setpoints keep their bar.** `ConsignePH`, `ConsigneRedox` and `ConsigneChlore` fall back to a read-only sensor precisely when the writable entity is refused. Lifting the bar for them would **replace** `sensor.ph_setpoint` with `number.ph_setpoint` on every installation carrying a sentinel — a deleted entity and a broken automation, which is [#128](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/128) and [#135](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/135) again. `ConsigneEau` has no such fallback, so barring it deleted an entity into nothing at all. That asymmetry is the fix, not a shortcut.
  - **It also closes a gap in the opposite direction.** The thermostat used to offer a target-temperature dial on hardware whose `HeaterMode` carries no setpoint, because the dial was derived from the reading rather than from the permission. That gate now actually fires.
  - ⚠️ **This does not replicate the official client's `SetParam`-before-`SetOut` ordering**, and that is deliberate: only the responses were captured, never the request body, so where v1 gets the value it writes is still unknown. Inventing one is the failure this project keeps refusing. After this fix the value comes from the user, and none is invented.
  - ⚠️ Only `-2000` was measured. `-1000` ("unknown") is treated the same because barring it removed nothing either — on `ConsigneEau` it produced no entity at all.

### Verified

- **477 tests** (up from 463): nine new for the `newState` table, five more for the setpoint guard.
...
- 🔴 **The setpoint guard is witnessed in both directions too**, and the two mutants redden **disjoint** sets of tests — which is what proves the new condition carries weight rather than merely passing:
  - Restoring the unconditional bar (the old behaviour) reddens **five** cases, all of them the water setpoint: its `number` entity, its value mapping, the thermostat's dial and the write.
  - Removing the bar entirely reddens **four** *different* cases, none of them the water setpoint: the pH and Redox read-only sensors, and the two controls that prove a sentinel takes down its own setpoint and no neighbour. Without this second mutant, "the guard is narrow" would be an assertion no test could falsify.
- **Negative control**: a `HeaterMode` of 3 — on/off heating without a setpoint — still withholds the dial and still refuses the write. The measurement refuted the sentinel, not [#124](https://forgejo.dragonlance.xyz/JonBasse/ha-klereo/issues/124).
