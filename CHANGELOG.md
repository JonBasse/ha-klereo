...
### Verified
- **533 tests** (up from 477): twenty-two new for the button, twenty-eight for the energy sensors, six for the disjoint gates.
- 🔴 **The disjoint-gate guard is discriminant against the CAREFUL alignment, which is the one that would have shipped.** Four mutants were run. The two blunt ones are already caught elsewhere by accident — barring `number` on a missing output 4 reddens **17** cases (2 here), and gating `climate` on the setpoint reddens **8** (3 here) — because unrelated fixtures happen to carry no outputs, or no `HeaterMode`. The two written the way this codebase actually reasons are caught **here and nowhere else**: honouring "an unknown answer never gates" and barring the setpoint only when the payload *does* report its outputs reddens **2 cases, both in `test_disjoint_gates.py`**, with the other 531 green; widening `climate` to a known `HeaterMode` with no output 4 reddens **exactly 1**, likewise only here. Without this file both would have passed CI.
